...
export async function createSocketHandlers(app) {
    ...
    for (const filename of filenames) {
        ...
        if (!module)
            throw new Error(`Failed to load module at path '${filename}'. Please ensure the file exists and is valid.`);
        if (!module.default)
            throw new Error(`Failed to load socket command module at path '${filename}'. The module must have a default export that can be used as a WebSocket command.`);
